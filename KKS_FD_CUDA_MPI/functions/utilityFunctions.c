...
void testThermoFuncs(domainInfo simDomain, simParameters simParams)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        /*
        Serial Directive: For sections where function pointers are used and file operations are performed, I used #pragma acc serial to ensure correct execution as these operations cannot be parallelized safely.        
        */
...
{
    *arr = (double*)malloc(sizeof(double)*N*stride);
...
void freeVars(domainInfo *simDomain, controls *simControls, simParameters *simParams)
{
    ...
    free(simDomain->thermo_phase_dev);
